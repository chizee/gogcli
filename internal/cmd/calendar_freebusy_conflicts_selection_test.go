...
func TestCalendarConflictsCmd_AllCalendarsSelection(t *testing.T) {
	assertCalendarConflictsUseAllCalendars(t, []string{"--all"})
}

func TestCalendarConflictsCmd_DefaultsToAllCalendars(t *testing.T) {
	assertCalendarConflictsUseAllCalendars(t, nil)
}

func assertCalendarConflictsUseAllCalendars(t *testing.T, selector []string) {
	t.Helper()
	var gotIDs []string
	srv := httptest.NewServer(withPrimaryCalendar(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		path := strings.TrimPrefix(r.URL.Path, "/calendar/v3")
